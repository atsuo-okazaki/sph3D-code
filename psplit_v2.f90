...
!!      IF (myrank == 0) THEN
!!         WRITE (iprint,*) nkillcur,' accreted/killed particles'
!!      ENDIF
      IF (nkillcur > 0) THEN
         ...
         ENDDO
!!         IF (myrank == 0) THEN
!!            WRITE (iprint,*) ii,' particles with iphase=-1'
!!         ENDIF
      ENDIF

!---- Only allow accretion of GAS particles evaluated 
...
!!      IF (nsplit /= 0) THEN
!!         IF (myrank == 0) THEN
!!            WRITE (*,*) nsplit,' split particles --> ', &
!!               iadd,' children particles'
!!         ENDIF
!!      ENDIF
